...
class CollisionSystem extends ECS.System {
	...
	updateEntity(entity: ECS.Entity, params: ECS.UpdateParams): void {
		...
		const possible = quadtree.query(rect);

		for (const target of possible) {
			if (target.id == rect.id) continue;

			if (velocity) {
				let { collision, contact_normal, contact_point, exit_point, time } = ECS.DynamicRectVsRect(
					rect,
					target,
					dt
				);
		...
		if (mouse) {
			...
			const target = new ECS.Vector(input.mouseX - input.lastX, input.mouseY - input.lastY);

			params.context.strokeStyle = "black";
			params.context.beginPath();
			params.context.moveTo(input.lastX, input.lastY);
			params.context.lineTo(input.mouseX, input.mouseY);
			params.context.stroke();
